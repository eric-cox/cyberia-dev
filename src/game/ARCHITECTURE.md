...
#### Рендеринг улиц
- **`render/StreetRenderer.js`** — упрощённая отрисовка улиц в один проход (KISS):
  - Базовое покрытие (асфальт, тротуары, площади)
  - Разметка (осевые линии)
  - Бордюры
  - Инфраструктура (люки)
  - Повреждения (лужи)
- Без кэширования слоёв (YAGNI) — карта генерируется один раз.

#### Визуал зданий и объектов
- **`render/TerrainPainter.js`** — отрисовка крыш зданий (вид сверху), префабов.
  Интегрирует `StreetRenderer` для отрисовки улиц.
  - Крыши: бортики только по внешнему периметру здания (не между тайлами), антенны (30%) и вентиляционные детали (20%) рисуются только на первом тайле здания
