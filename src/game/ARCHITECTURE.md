...
| world | `world/tiles.js`, `world/WorldMap.js`, `world/WorldGen.js`, `world/prefabs.js` | Реестр ячеек (speed/inertia/solid); данные карты + запросы; генерация района (5 шагов); каталог префабов (уличные объекты) |
...
### Карта (городской район, префабы, процедурная генерация)
- **Система ячеек** — `world/tiles.js` хранит `TILE_TABLE`, у каждой ячейки
...
- **Генератор района** — `world/WorldGen.js` (5 шагов):
  1. Разметка зданий (блоки 5×5 – 15×15, минимум 2 тайла между ними)
  2. Стены и двери (периметры зданий + 1-3 двери на здание)
  3. Валидация связности (Flood Fill — все улицы связаны)
  4. Расстановка объектов (префабы вдоль стен и на улицах)
  5. Проверка коллизий (ширина проходов ≥ 2 тайла)
- **Префабы** — `world/prefabs.js`: каталог уличных объектов с коллизиями,
  тегами и неоновыми точками. Новые объекты = новый префаб в каталоге.
- **Визуал** — `render/TerrainPainter.js`: отрисовка стен зданий, префабов,
  неоновых свечений.
