...
- Генераторы — `world/WorldGen.js`: новый генератор возвращает `WorldMap`.
  ...
  с фьордами и тупиками; угловой шум делает контур неправильным). По периметру
  карты — непроходимая стена толщиной 3 тайла (фаза 1.5). Внутри —
  снег 3 глубин (к центру мельче, к краю глубже), блоки домов (от 3×3 до 6×6
  тайлов) с окнами на фасадах и неоновыми вывесками (чаще у края), замёрзшие
  озёра, мёртвые деревья.
