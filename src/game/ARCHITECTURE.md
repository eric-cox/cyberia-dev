...
| world | `world/tiles.js`, `world/WorldMap.js`, `world/WorldGen.js`, `world/prefabs.js`, `world/streetTypes.js`, `world/streetPrefabs.js`, `world/streetNetwork.js` | Реестр ячеек (speed/inertia/solid); данные карты + запросы; генерация района (6 этапов); каталог префабов (уличные объекты); система типов улиц и их параметров; каталог префабов улиц; 4-фазный алгоритм генерации уличной сети |
...
#### Система ячеек
- **`world/tiles.js`** хранит `TILE_TABLE`, у каждой ячейки `speed` (0..1, 0 = непроходимо)
...
- **`world/WorldMap.js`** (`get/set/cellAt/collidesCircle/freeSpot`). Чанковая подгрузка
  реализуется внутри класса, потребители не меняются.

#### Генерация района (6 этапов)
- **`world/WorldGen.js`** — главная функция генерации:
  1. Генерация уличной сети (4 фазы через `StreetNetwork`)
  2. Конвертация уличной сети в тайлы карты
  3. Генерация зданий в зонах застройки
  4. Расстановка уличных объектов (префабы)
  5. Добавление деталей (люки, лужи, мусор)
  6. Финализация (стартовая поляна, индекс проходимых ячеек)

#### Система уличной сети
- **`world/streetTypes.js`** — классификация улиц (магистрали, переулки, пешеходные зоны),
  направления, бордюры.
- **`world/streetPrefabs.js`** — каталог префабов улиц (перекрёстки, повороты, тупики).
- **`world/streetNetwork.js`** — 4-фазный алгоритм генерации:
  1. Закладка магистралей (jittered grid, 2-3 вертикальных + 2-3 горизонтальных)
  2. Добавление переулков (30% тупиков, L-образные повороты)
  3. Вставка площадей (8×8 – 15×15, органичная форма)
  4. Финализация границ (BUILDING_ZONE, детали)
- **Многослойная структура данных**: `streetType`, `streetDir`, `curbSide`, `decoration`
  (типизированные массивы для производительности).

#### Префабы уличных объектов
- **`world/prefabs.js`** — каталог объектов с коллизиями, тегами и неоновыми точками.
  Новые объекты = новый префаб в каталоге.

#### Многослойный рендеринг улиц
- **`render/StreetRenderer.js`** — 5 слоёв отрисовки:
  1. **Ground Layer** — базовое покрытие (асфальт, тротуары, площади, бордюры)
  2. **Markings Layer** — разметка (осевые линии, разделительные полосы, пешеходные переходы)
  3. **Infrastructure Layer** — инфраструктура (люки, ливневые решётки, кабельные каналы)
  4. **Decay Layer** — повреждения и мусор (трещины, лужи, мусор, подтёки)
  5. **Neon Layer** — неон (динамический слой с мерцанием и отражениями в лужах)
- **Оптимизация**: слои 1-4 кэшируются в offscreen canvas, слой 5 перерисовывается каждый кадр.
- **Отражения неона**: рассчитываются один раз при генерации карты (статический эффект).

#### Визуал зданий и объектов
- **`render/TerrainPainter.js`** — отрисовка стен зданий, префабов, неоновых свечений.
  Интегрирует `StreetRenderer` для отрисовки улиц.
