...
const PROFILES = {
  default: {
    ...
    enemies: {
      ...
      // Дробные значения = вероятность: ИИ-бот [0.3, 0.3] —
      ...
      // Охрана лута: вокруг каждого артефакта на карте —
      // скопление врагов (count особей в кольце ring тайлов).
      lootGuard: {
        count: [2, 4],
        ring: [2, 4],
    ...
    loot: {
      ...
      ammoCount: 13, // россыпей патронов на карте
      ammoRing: [8, 50], // где они лежат
      ammoPerPickup: 2, // патронов в одной россыпи
      ...
      autoEquipMaxTier: 1,
      // На карте лежат только УЛУЧШЕНИЯ надетого: от 0 до 2 шт.,
      // каждый чуть сильнее текущего (см. loot/RunLoot.placeUpgrades).
      upgradeCountRange: [0, 2],
      // Запасной путь к победе, когда улучшений на карте нет:
      // победа даётся за достижение этого уровня.
      victoryLevel: 5,
    },
